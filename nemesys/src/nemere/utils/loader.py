...
class CSVSpecimenLoader(BaseLoader):


    def __init__(self, csvfile: str, layer:int=-1, relativeToIP:bool=False):
        """
        Load the messages from the CSV file of the given name.

        :param csvfile: CSV input file name.
        """
        self.csvFileName = csvfile
        self.messagePool = OrderedDict()

        # Read data using an internal method
        hex_lists, labels = self.read_data(csvfile)
        # Process each message-label pair into l5msgs and l1msgs
        l5msgs = [RawMessage(payload) for payload in hex_lists]
        
        l1msgs = [RawMessage(payload) for payload in hex_lists]
        
        """
        for hex_list in hex_lists:
            raw_message = RawMessage(hex_list)
            # If AbstractMessage can accept a label, use it here; otherwise, extend RawMessage or create a subclass
            l5msg = AbstractMessage(raw_message)  # Adjust this line based on your AbstractMessage implementation
            
            l5msgs.append(l5msg)
            l1msgs.append(raw_message)  # Use the same raw message for both l5msgs and l1msgs
        """
        super().__init__(l5msgs, l1msgs)

    def read_data(self, filename):
        """
        Reads data from a CSV file and returns lists of byte-converted hex strings and their labels.
        
        :param filename: The path to the CSV file.
        :return: A tuple of two lists (hex_lists, labels).
        """
        hex_lists = []
        labels = []
        ...
        with open(filename, mode='r', newline='') as file:
            ...
            for row in reader:
                ...
                hex_list = bytes.fromhex(hex_string.strip())
                hex_lists.append(hex_list)
                labels.append(label)

        return hex_lists, labels
