...
def cluster_core_samples(dist_matrix, core_samples, algorithm='mix'):
    if algorithm.lower() == 'hdbscan':
        ...
        print(f"Best threshold: {best_threshold}, Best Silhouette Score: {best_silhouette_score}")
...
def hybrid_clustering(samples, num_core_samples, distance_function, raw_data, algorithm='hdbscan'):
...
if __name__ =="__main__": 
    ...
    for i in range(10):
        ...
        c_final.append(c)
        v_final.append(v)
    print(f"h:{sum(h_final) / len(h_final)}")
    print(f"c:{sum(c_final) / len(c_final)}")
    print(f"v:{sum(v_final) / len(v_final)}")
    print("Done!!!")
