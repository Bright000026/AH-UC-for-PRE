...
def  calcBEMeasuresOfSlice(WINDOW,xs):
	...
	for x in xs:
		assert(len(x)==WINDOW)
		#传进x应该是4bytes？
		#prophecyBEF以字典的形式返回x中的符号位，指数位和最后几位
		#{"sign":be_i_sign,"exp":be_i_exp,"exp_byte":be_i_exp_byte,"mant":be_i_mant,"mant_byte":be_i_mant_byte,"mant_cor":be_i_mant_cor,"f":f,"b0":xs[0],"b1":xs[1],"b2":xs[2],"b3":xs[3]}
		m = prophecyBEF(x)
		for k in m:
			cols[k].append(m[k])
    #cols是一个defaultdict对象，返回的是类似m的字典，索引值对应的对象以列表形式返回，不在m中的索引值返回空列表
	return cols

def var(xs,f):#计算xs的方差
	#v = statistics.variance(xs)
	#
	xs_ = [np.frombuffer(struct.pack("<f",x),dtype=np.float32) for x in xs]
	v = np.var(xs_)#用np中的var函数进行计算
	#v = statistics.variance(xs)#再用statistics库进行计算？？？
	if np.isfinite(v):
